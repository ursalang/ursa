...
export class ArkLocalInst extends ArkSymInst {
  constructor(sourceLoc: Interval | undefined, public index: number) {
    super(sourceLoc)
  }
}
...
export function flattenExp(
  ...
  if (exp instanceof ArkLiteral) {
    ...
    return new ArkInsts([new ArkLocalInst(exp.sourceLoc, exp.index)])
  } else if (exp instanceof ArkCapture) {
    return new ArkInsts([new ArkCaptureInst(exp.sourceLoc, exp.index)])
  } else {
