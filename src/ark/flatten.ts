  ...
  function doExpToInsts(
    ...
    if (exp instanceof ArkLiteral) {
      ...
      for (const bv of exp.boundVars) {
        const bvInsts = doExpToInsts(bv.init, innerLoop, innerFn, bv.name)
        insts.push(
          ...bvInsts.insts,
      ...
      return new ArkInsts([new ArkLocalInst(exp.sourceLoc, exp.index, exp.id.name)])
    } else if (exp instanceof ArkCapture) {
      return new ArkInsts([new ArkCaptureInst(exp.sourceLoc, exp.index, exp.id.name)])
    } else {
