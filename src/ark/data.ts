...
ArkStringType.implement(
  new ArkTrait(
    'StringTrait',
    new Map([
      ['len', new ArkMethodType(new ArkFnType(false, [], ArkNumberType))],
  ...
  new ArkImpl(new Map<string, ArkCallable>([
    ['len', new NativeFn([new ArkTypedId('self', ArkSelfType)], ArkNumberType, (thisVal: ArkStringVal) => ArkNumber(thisVal.val.length))],
...
export function toJs(val: ArkVal): unknown {
  if (val instanceof ArkConcreteVal) {
    ...
    for (const impl of val.type.impls.values()) {
...
export const globalTypes = new Namespace<ArkType>([
