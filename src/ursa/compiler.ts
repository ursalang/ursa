...
semantics.addOperation<LetBinding>('toLet(a)', {
  Lets(lets) {
    ...
    const locations = letIds.map(
      (id, n) => new Location(id, ArkUndefinedVal, letVars[n]),
    )
...
semantics.addOperation<ArkExp>('toExp(a)', {
  ...
  Fn(ty, body) {
    ...
    return addLoc(new CodeConstructor(
      fnType.params,
      fnType.returnType,
  ...
  For(_for, ident, _in, iterator, body) {
    ...
    const innerEnv = this.args.a.env.push([new Location('$iter', ArkVal, false)])
    const compiledIterator = iterator.toExp({...this.args.a, env: innerEnv})
    // FIXME: type of iterVar: return type of $iter
    const loopEnv = innerEnv.push([new Location(iterVar, ArkVal, false)])
    ...
    const loopBody = addLoc(
      new ArkLet(
        ...
        [new ArkBoundVar(iterVar, ArkVal, false, innerIndex, addLoc(new ArkCall(addLoc(symRef(loopEnv, '$iter'), iterator), []), this))],
        new ArkSequence([
          new ArkIf(
    ...
    return addLoc(
      new ArkLet([new ArkBoundVar('$iter', ArkVal, false, localsDepth, compiledIterator)], new ArkLoop(loopBody, localsDepth + 1)),
      this,
    )
  },
