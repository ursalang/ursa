...
import {Environment, TypedLocation, compile as arkCompile} from '../ark/reader.js'
  ...
  for await (const line of rl) {
    try {
      ...
      if (compiled instanceof ArkLet) {
        env = env.push(compiled.boundVars.map(
          (bv) => new TypedLocation(bv.name, bv.type, bv.isVar),
        ))
